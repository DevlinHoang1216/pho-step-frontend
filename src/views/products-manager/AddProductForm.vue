            ...
            <div v-if="selectedSizes.length > 0" class="selected-sizes-preview">
              ...
              >
                ...
                <CButton
                  ...
                  class="ms-1 remove-custom-size-btn"
...
export default {
 ...
 async loadAllInitialData() {
    ...
    async addCustomColor() {
      ...
      // Đặt tên tạm thời hoặc yêu cầu người dùng nhập tên cho màu tùy chỉnh
      const timestamp = Date.now();
      const tempName = `Tùy chỉnh ${hex}`;
      const tempMaMauSac = `CUSTOM_${timestamp}`; // Mã màu sắc tạm thời, duy nhất

      // Kiểm tra trùng lặp dựa trên HEX (để tránh thêm trùng màu)
      if (
        this.colorHistory.some((c) => c.hex === hex)
      ) {
        this.toast.warning('Màu này đã tồn tại trong lịch sử hoặc đã được thêm vào hệ thống.')
        return;
      }
      ...
      try {
        ...
        const newColor = {
          id: newColorFromApi.id, // Sử dụng ID từ Backend
          hex: newColorFromApi.hex,
          name: newColorFromApi.tenMauSac, // Tên được Backend trả về
          maMauSac: newColorFromApi.maMauSac,
        };

        this.colorHistory.unshift(newColor); // Thêm vào đầu lịch sử
        // Cập nhật lại availableColors để màu mới xuất hiện trong danh sách hệ thống
        this.availableColors.push(newColor);

        this.toast.success(`Đã thêm màu tùy chỉnh: ${newColor.name}`);

        // Tự động chọn màu này sau khi thêm
        ...
        // Nếu lỗi do trùng mã màu sắc (maMauSac) mà BE đã kiểm tra, hiển thị thông báo
        if (error.response && error.response.data && error.response.data.message) {
            this.toast.error(error.response.data.message); // Hiển thị thông báo lỗi từ BE
        } else {
    ...
    async deleteColor(colorId) {
      if (confirm('Bạn có chắc chắn muốn xóa màu này không?')) {
        try {
          // Kiểm tra xem màu có phải là màu từ DB không (id dương)
          // Hiện tại, tất cả màu sẽ có id dương nếu được thêm thành công vào DB
          // Các màu fallback có id âm, nhưng không thể xóa khỏi DB
          if (colorId > 0) {
            await axios.delete(`http://localhost:8080/api/mauSacs/${colorId}`)
            this.toast.success('Đã xóa màu khỏi hệ thống.')
          } else {
            // Nếu là màu fallback (ID âm), chỉ xóa khỏi lịch sử Frontend nếu cần
            // (Thực tế không cần xóa màu fallback khỏi lịch sử, chỉ quản lý màu tùy chỉnh)
    ...
    async addCustomSize() {
        ...
        try {
            const formattedSizeValue = newSizeValue.replace('.', '_');
            const randomSuffix = Math.floor(Math.random() * 9000) + 1000;
            const maKichCo = `SIZE_${formattedSizeValue}_${randomSuffix}`;
            ...
            const newSize = {
                id: response.data.id,
                value: response.data.tenKichCo,
                maKichCo: response.data.maKichCo,
                isCustom: true,
            };
